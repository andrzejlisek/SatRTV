...
namespace SatRTV
{
	...
	{
        string[] ParseTable(HtmlAgilityPack.HtmlNode TableN, int _i, StreamWriter FS1W)
        {
            string[] SatTrans = null;
            if (TableN.Attributes["class"] != null)
            {
                if (TableN.Attributes["class"].Value == "frq")
                {
                    for (int _ii = 0; _ii < TableN.ChildNodes.Count; _ii++)
                    {
                        if (TableN.ChildNodes[_ii].Name == "tr")
                        {
                            int RowNum = (_i * 1000000) + (_ii * 1000);
                            int TdNum = 0;
                            SatTrans = new string[14];
                            for (int _iii = 0; _iii < TableN.ChildNodes[_ii].ChildNodes.Count; _iii++)
                            {
                                HtmlAgilityPack.HtmlNode NN = TableN.ChildNodes[_ii].ChildNodes[_iii];
                                if (NN.Name == "td")
                                {
                                    switch (TdNum)
                                    {
        ...
        void ParseDiv(HtmlAgilityPack.HtmlNode DivN, int _i, StreamWriter FS2W, bool TypeByPID, string[] SatTrans)
        {
            if (DivN.Attributes["id"] != null)
            {
                if (DivN.Attributes["id"].Value.StartsWith("m"))
                {
                    for (int _ii = 0; _ii < DivN.ChildNodes.Count; _ii++)
                    {
                        HtmlAgilityPack.HtmlNode NN = DivN.ChildNodes[_ii];
                        if (NN.Name == "table")
                        {
